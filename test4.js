...
function computerMakeArt() {
    ...
    for (var i = totalCircles.length - 1; i >= (totalCircles.length - 2); i--) {
        ...
        for (var index = 0; index < anglePoints.length - 1; index++) {
            ...
            //ctx.ellipse(totalCircles[i].posX, totalCircles[i].posY, totalCircles[i].radius, totalCircles[i].radius, 45 * Math.PI / 180, 0, 2 * PI);
            ctx.moveTo(startX, startY);
            ctx.bezierCurveTo(0, 0, startX, startY, endX, endY);
            ctx.moveTo(posX, posY);
            ctx.quadraticCurveTo(startX, startY, endX, endY);
            ctx.moveTo(endX, endY);
            ctx.quadraticCurveTo(posX, posY, width, height / 2);
            ctx.quadraticCurveTo(posX, posY, 0, height / 2);
        }
